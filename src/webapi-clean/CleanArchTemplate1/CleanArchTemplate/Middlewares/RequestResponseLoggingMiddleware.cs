...
namespace CleanArchTemplate.Middlewares;

public class RequestResponseLoggingMiddleware
{
    public RequestResponseLoggingMiddleware(RequestDelegate next, ILogger<RequestResponseLoggingMiddleware> logger, IOptions<TemplateOptions> TemplateOptions)
    {
        _next = next;
        _logger = logger;
        _recyclableMemoryStreamManager = new RecyclableMemoryStreamManager();
        _CleanArchTemplateApiOptions = TemplateOptions.Value;
    }

    private readonly TemplateOptions _CleanArchTemplateApiOptions;
    ...
    private async Task<string> FormatResponse(HttpContext context, MemoryStream responseBody)
    {
        ...
        string text = _CleanArchTemplateApiOptions.EnableOkResponseLogging ? await new StreamReader(responseBody).ReadToEndAsync() : string.Empty;
        responseBody.Seek(0, SeekOrigin.Begin);
